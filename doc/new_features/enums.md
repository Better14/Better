...
`enum` is a **contextual keyword**: it starts a top-level enum declaration (`enum Name { ... }`) but remains a normal identifier elsewhere (parameter names, range variables, struct fields, etc.). This keeps third-party code that uses `enum` as an identifier compatible with Better.
...
- Enums are distinct from `int?` nilable types and from `iota` constant groups.
