# Installing Bow from source

Bow is built the same way as upstream Go: a **bootstrap Go toolchain** compiles the sources in this tree, then reinstalls the result as your new `GOROOT`. The process follows [Installing Go from source](https://go.dev/doc/install/source); this document covers Bow-specific paths and Windows prerequisites.

## Overview

1. Install a **bootstrap** Go binary (upstream, not Bow) — [go.dev/dl](https://go.dev/dl/).
...
4. Set **`GOROOT`** and **`PATH`** to use Bow’s `bin/go`.
...
- Set **`GOROOT_BOOTSTRAP`** to the root of your upstream install if `go` on `PATH` points at Bow or is missing.
- **`GOROOT_BOOTSTRAP`** must **not** be the Bow tree you are building.
...
Add those exports to your shell profile (`~/.bashrc`, `~/.zshrc`, etc.) to keep Bow as the default.
...
Bow’s Windows build expects a C toolchain on `PATH` (typically **`gcc.exe`**) for parts of the runtime and cgo. Install **MinGW** with the **MSYS Basic System** and ensure `gcc` is available.
...
| Variable | Purpose |
| -------- | ------- |
| `GOROOT_BOOTSTRAP` | Upstream Go tree used to compile Bow (must contain `bin/go` or `bin\go.exe`) |
| `GOROOT` | Root of the **built** Bow tree (set after install) |
...
Build [gopls](doc/new_features/gopls.md) from the `go_tools` repository against Bow’s `GOROOT`:
