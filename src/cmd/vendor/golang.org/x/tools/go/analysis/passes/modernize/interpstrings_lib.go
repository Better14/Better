...
func rewriteSprintfToInterp(fset *token.FileSet, f *ast.File, edits []textEdit) ([]textEdit, int) {
	...
	ast.Inspect(f, func(n ast.Node) bool {
		...
		edits = append(edits, textEdit{start: start, end: end, text: []byte(text)})
		count++
		return true
	})
	return edits, count
...
func rewriteConcatToInterp(fset *token.FileSet, f *ast.File, edits []textEdit) ([]textEdit, int) {
...
func escapeLiteralBraces(fset *token.FileSet, f *ast.File, edits []textEdit) ([]textEdit, int) {
	skip := byteSliceStringLits(f)
	count := 0
	ast.Inspect(f, func(n ast.Node) bool {
		...
		edits = append(edits, textEdit{start: start, end: end, text: []byte(escaped)})
		count++
		return true
	})
	return edits, count
