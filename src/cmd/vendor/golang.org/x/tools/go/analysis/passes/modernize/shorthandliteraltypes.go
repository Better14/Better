...
// shorthandSliceRewriteOK reports whether []T{…} can drop its type prefix.
// The rewrite is allowed only when shorthand inference would pick the same
// element type T (e.g. []int{1,2} yes, []int64{1,2} no).
...
// shorthandMapRewriteOK reports whether map[K]V{…} can drop its type prefix.
...
// shorthandSetOfRewriteOK reports whether set.Of(…) can become {…}.
...
func setOfTypeArg(call *ast.CallExpr) (ast.Expr, bool) {
	...
	case *ast.IndexExpr:
		if sel, ok := fun.X.(*ast.SelectorExpr); ok && isSetPackageIdent(sel.X) && isSetOfIdent(sel.Sel) {
