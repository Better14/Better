...
func (check *Checker) updateExprType(x syntax.Expr, typ Type, final bool) {
	...
	case *syntax.SwitchExpr:
		for _, c := range x.Body {
			if c != nil {
				check.updateExprType(c.Body, typ, final)
			}
		}
	...
	if _, ok := typ.Underlying().(*Optional); ok && val != nil {
		// Nilable values are lowered to Option[T] structs; not compile-time constants.
		mode = value
		val = nil
	}
	...
	if typ != nil {
		if u, _ := commonUnder(typ, nil); u != nil {
			if sig, _ := u.(*Signature); sig != nil {
				return &target{sig: sig, desc: desc}
			}
		}
	}
	return nil
