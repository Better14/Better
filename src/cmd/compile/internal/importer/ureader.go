...
func (r *reader) param() *types2.Var {
	...
	v := types2.NewParam(pos, pkg, name, typ)
	if r.Version().Has(pkgbits.ParamDefaultVal) && r.Bool() {
		v.SetDefaultVal(r.Value())
	}
	return v
}
...
// extensionImportShape reports whether typ is the first parameter type of an
// exported extension (receiver lowered to a parameter).
func extensionImportShape(defPkg *types2.Package, typ types2.Type) bool {
	for {
