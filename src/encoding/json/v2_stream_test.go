...
func TestDecodeInStream(t *testing.T) {
	tests := []struct {
		...
		{CaseName: Name(""), json: `{ "` + strings.Repeat("a", 513) + `" 1 }`, expTokens: []any{
			...
			decodeThis{&SyntaxError{msg: "invalid character '1' after object key", Offset: len64(`{ "` + strings.Repeat("a", 513) + `" `)}},
		}},
		{CaseName: Name(""), json: `{ "\a" }`, expTokens: []any{
			Delim('{'),
			&SyntaxError{msg: "invalid escape sequence `\\a` in string", Offset: len64(`{ "`)},
		}},
		{CaseName: Name(""), json: ` \a`, expTokens: []any{
...
func TestTokenError(t *testing.T) {
	tests := []struct {
		...
		{in: `[0. `, err: &SyntaxError{msg: "invalid character ' ' in numeric literal", Offset: int64(len(`[0.`))}},
		{in: `[0,`, err: io.EOF},
