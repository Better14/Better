	...
	if raw := t.raw; raw != nil {
		...
		case 'f':
			...
			if bits == 32 && !math.IsInf(f64, 0) && math.IsInf(float64(float32(f64)), 0) {
				return f64, newNumError("Float", t.String(), strconv.ErrRange)
			}
			return f64, nil
		case 'i':
...
func (t Token) Int() (int64, error) {
	if raw := t.raw; raw != nil {
		...
		if len(buf) > 0 && buf[0] == '-' {
			...
			case abs > -minInt64:
				return minInt64, newNumError("Int", t.String(), strconv.ErrRange)
			case ok:
			...
			case abs > +maxInt64:
				return maxInt64, newNumError("Int", t.String(), strconv.ErrRange)
			case ok:
				return +1 * int64(abs), nil
			}
		}
		...
		if Kind(buf[0]).normalize() == '0' {
			...
			return f64toi64(f64), newNumError("Int", t.String(), strconv.ErrSyntax)
		}
	} else if t.num != 0 {
		// Handle typed Go number value.
		switch t.str[0] {
		...
		case 'f', 'F':
			...
			case math.IsNaN(f64), math.Trunc(f64) != f64:
				return i64, newNumError("Int", t.String(), strconv.ErrSyntax)
			case (i64 == minInt64 && f64 < minInt64) || (i64 == maxInt64 && f64 > maxInt64):
				return i64, newNumError("Int", t.String(), strconv.ErrRange)
			default:
				return i64, nil
			}
		}
...
func (t Token) Uint() (uint64, error) {
	...
	if raw := t.raw; raw != nil {
		...
		case abs == maxUint64: // implies overflows
			return maxUint64, newNumError("Uint", t.String(), strconv.ErrRange)
		}
		// This is not an unsigned integer, which implies ErrSyntax.
		if Kind(buf[0]).normalize() == '0' {
			f64, _ := strconv.ParseFloat(string(buf), 64)
			return f64tou64(f64), newNumError("Uint", t.String(), strconv.ErrSyntax)
		}
	} else if t.num != 0 {
		// Handle typed Go number value.
		switch t.str[0] {
		...
		case 'i':
			if int64(t.num) < minUint64 {
				return minUint64, newNumError("Uint", t.String(), strconv.ErrSyntax)
			}
		...
		case 'f', 'F':
			...
			case math.IsNaN(f64), math.Trunc(f64) != f64, math.Signbit(f64):
				return u64, newNumError("Uint", t.String(), strconv.ErrSyntax)
			case (u64 == minUint64 && f64 < minUint64) || (u64 == maxUint64 && f64 > maxUint64):
				return u64, newNumError("Uint", t.String(), strconv.ErrRange)
			default:
				return u64, nil
			}
		}
