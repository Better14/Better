...
// ErrorPlusV is optionally set by fmt.init to format structured errors with %+v.
var ErrorPlusV func(error) (string, bool)
	...
	if p.fmt.sharpV {
		...
		case 'v', 's', 'x', 'X', 'q':
			...
			case error:
				...
				if ErrorPlusV != nil {
					if msg, ok := ErrorPlusV(v); ok && p.fmt.plusV && verb == 'v' {
						p.fmtString(arg, value, msg, verb)
						return
					}
				}
