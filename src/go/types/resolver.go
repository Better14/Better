...
func (check *Checker) ensureImported(pos token.Pos, path string) *PkgName {
	...
	if len(check.files) > 0 {
		fileDir = dir(check.fset.Position(check.files[0].Name.Pos()).Filename)
	}
	imp := check.importPackage(atPos(pos), path, fileDir)
	if imp == nil || imp.fake {
		return nil
	}
	found := false
