...
func NewOpError(op, net string, source, addr Addr, err error) *OpError {
	...
	errors.InitCustom(&e.Base, "%s", opErrorMessage(op, net, source, addr, err))
	return e
}
...
func NewParseError(typ, text string) *ParseError {
	...
	errors.InitCustom(&e.Base, "%s", parseErrorMessage(typ, text))
	return e
}
...
func NewAddrError(err, addr string) *AddrError {
	...
	errors.InitCustom(&e.Base, "%s", addrErrorMessage(err, addr))
	return e
}
...
func newDNSError(err error, name, server string) *DNSError {
	...
	errors.InitCustom(&de.Base, "%s", dnsErrorMessage(de))
	return de
}
...
func dnsError(errMsg, name string) *DNSError {
	...
	errors.InitCustom(&e.Base, "%s", dnsErrorMessage(e))
	return e
}
